...
| Проблема | Деталі |
|----------|--------|
| **bright spike** | bright=2→23→2 → pose +13м за один інтервал. Яскравий об'єкт у кадрі (пакет, лампа). Перед LOITER: прибрати яскраві об'єкти з поля зору камери |
| y VO bias | ~0.01м/5с залишковий y drift = нахил камери, dead-zone не допомагає — прийнятно для EKF3 |
...
| Пріоритет | Задача |
|-----------|--------|
| **NEXT** | LOITER тест з пеноплексом — drift 0.35м/хв має бути прийнятним для EKF3. Перед тестом: усунути яскраві об'єкти з поля зору камери (bright spike = LOITER killer) |
