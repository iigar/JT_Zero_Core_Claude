...
57. **Систематичний IMU bias drift (~0.01м/5с без моторів) — dead-zone в Kalman predict** — IMU predict step `kf_vx_ += imu_ax_ * dt` (Fix #41) накопичує навіть мізерний статичний bias (~0.004 м/с²) у позиційний drift. Hover decay (0.85/frame) частково компенсує, але steady-state залишається ненульовим. Симптом: рівномірний -x/+y drift рівно ±0.01м/5с на нерухомому дроні без моторів, conf=0.58 (трекінг нормальний). Fix: dead-zone 0.05 м/с² на `imu_ax_`/`imu_ay_` перед predict — при `|acc| < 0.05 м/с²` predict крок не виконується. Реальні маневри >> 0.1 м/с², тому корисний сигнал не обрізається. `camera_pipeline.cpp:Phase2 IMU predict`.
