...
68. **Гравітаційний витік у IMU predict — VO-дрейф "прибитий" до одного напрямку, фейковий масштаб ×20 при русі** — Нахил рами лише на 2.4° "зливає" гравітацію в горизонтальні осі акселерометра FC: acc=(-0.36,-0.41) м/с² константно (доведено на залізі: збіг з -9.81·sin(tilt) до 3-го знаку; напрямок acc-вектора 228.6° = напрямок VO-дрейфу). Це 4× вище IMU_ACCEL_DEADZONE=0.10 → predict `kf_vx_ += imu_ax·dt` (Fix #41) постійно тягнув Kalman-швидкість у напрямку нахилу → `result.dx = kf_vx·dt` прибитий до однієї діагоналі незалежно від реального руху (статика: hover decay стримував до ~0.9 м/хв дрейфу; реальний рух: hover off → поза ×20). Bias-калібровка (Fix #60/61) безсила — віднімає з ВИМІРУ raw_v, отрута заходить через PREDICT. Перша спроба `acc += 9.81·sin(roll/pitch)` зі state_ ПОГІРШИЛА — конвенції/джерела state_.roll/pitch vs acc не збігаються. Робочий фікс: high-pass — повільний EMA (ACC_LP_ALPHA=0.02, ~3.3с при 15fps) відстежує DC-складову acc (витік гравітації + IMU bias за будь-якого монтажу), predict отримує лише AC-залишок; deadzone лишається. Верифіковано: lp=(-0.424,-0.469) зловив витік точно, hp≈0.003, kf=(0,0), дрейф позиції НУЛЬ за 30с. Ймовірно це ж — корінь історичного motor drift у LOITER (політ = постійний нахил). `camera.h:acc_lp_x_/y_/acc_lp_init_`, `camera_pipeline.cpp:Phase2 predict`.
67. **Камера догори ногами → інвертований VO → EKF3 "stopped aiding" цикл** — rpicam-vid команда не мала flip-прапорів. При монтажі камери на 180° зображення перевернуте → VO рахує зміщення фіч інвертовано (dx/dy зі зворотним знаком) → накопичена поза йде в протилежний бік → суперечить IMU → EKF3 відкидає external nav ("stopped aiding" цикл, позиція стрибає 1.4→-5.3→12.3м). Підтверджено візуально (кадр через /api/camera/frame). Fix: env-vars `JTZERO_HFLIP`/`JTZERO_VFLIP` (0/1) → додають `--hflip`/`--vflip` у rpicam-vid. 180° = обидва=1. `camera_drivers.cpp:PiCSICamera::open`. Разом з темрявою (bright 2-3 на дефолтних shutter=20000/gain=2.0) — підняти gain=4.0/shutter=33333 для темного приміщення.
66. **VISION_POSITION_ESTIMATE #102 не слався — EKF3 не фузив external-nav ПОЗИЦІЮ** — `tick()` слав лише ODOMETRY #331; `send_vision_position` (#102) був мертвий код (Fix #65 це виявив). ArduPilot 4.3.6 брав з #331 швидкість (velocity_horiz On) але НЕ встановлював горизонт. позицію (const_pos_mode On, pos_horiz Off). Fix: оживлено `send_vision_position(build_vision_position(...))` у tick() поряд з ODOMETRY. Підтверджено логами MP: "EKF3 IMU0/IMU1 is using external nav data", дрон рухається на мапі. (#102 несе Fix #65 covariance.) `mavlink_interface.cpp:tick()`.
65. **VO-дрейф не доходив до EKF3 covariance — EKF3 не знав про накопичений дрейф** — Fix #40 рахує `position_uncertainty` (накопичений дрейф), але воно йшло лише на дашборд. Covariance до EKF3 у ODOMETRY #331 бралась з `confidence` (миттєва якість), не з дрейфу. **ВАЖЛИВО:** `send_vision_position` (#102) — МЕРТВИЙ КОД, ніде не викликається; реальний external-nav у EKF3 = **ODOMETRY #331** (`tick()` шле лише `send_odometry`, попри старі згадки про #102 у доках). Fix: (a) clamp `position_uncertainty` до `[POS_UNC_FLOOR=0.2, POS_UNC_CAP=5.0]`м; `reset()` → `POS_VAR_INIT` (uncertainty=floor); (b) `MAVOdometry.position_uncertainty` з `vo.position_uncertainty`; (c) `send_odometry` pos_var = `max(drift_var, conf_var)` — дрейф робить EKF3 обережнішим, confidence зберігає anti-cycling (Fix #46). Cap 5м не дає EKF3 відкинути VO у GPS-denied. #102 теж отримав covariance (захисно, якщо колись увімкнуть). `camera.h:POS_UNC_FLOOR/CAP/VAR_INIT`, `camera_pipeline.cpp`, `mavlink_interface.cpp:build_odometry/send_odometry`. Валідація EKF3-споживання — бенч-тест з MAVLink Inspector (innovations).
76. **result.valid не мав абсолютного порогу яскравості — темний/заблокований кадр міг пройти як valid** — `position_update` (camera_pipeline.cpp) залежав лише від `running_confidence_ > 0.15f && inlier_count >= min_inliers`. Історія (Fix #58/#59) уже показала: у майже чорному кадрі (bright≈0-9) LK трекає сенсорний шум як впевнені кути — conf МОЖЕ рости до 0.65-0.70 саме тоді, коли зображення непридатне. Spike-фільтр (Fix #58/#59) ловить лише РАПТОВІ зміни відносно rolling average — камеру, яка стартує чи повільно стає темною, він не ловить. Fix: додано `frame_brightness_ >= MIN_VALID_BRIGHTNESS` (20.0f, узгоджено з `BRIGHT_DROP` з VO Fallback) до умови `position_update` — гейтить і накопичення пози (`pose_x_/y_`), і сам `result.valid` через існуючий debounce. Верифіковано на залізі: живий полінг під час поступового закриття об'єктива рукою показав bright 35→0.3 з conf, що ЗРОСЛА до 0.70 — і `valid` стабільно тримався `false` увесь цей час. `camera.h:MIN_VALID_BRIGHTNESS`, `camera_pipeline.cpp` (`position_update`).
77. **DEADLOCK у spike-детекторі (Fix #58/59) — після сталої зміни яскравості VO міг НАЗАВЖДИ лишитись invalid, до рестарту сервісу** — Виявлено під час верифікації Fix #76 на залізі. `bright_rolling_avg_` оновлюється лише на "не-spike" кадрах (`if (is_spike) {...} else { avg = 0.9*avg + 0.1*current; }`). Якщо яскравість робить СТІЙКУ (не миттєву) зміну — камера була темна довше кількох секунд, потім знову світла — кожен новий яскравий кадр класифікується як "bright spike" відносно застарілого низького `avg`, тому `avg` НІКОЛИ не оновлюється назад, і кожен наступний кадр знову ловиться як spike: самопідтримуваний нескінченний цикл. На залізі підтверджено: `bright_rolling_avg_` застряг на 3 після 10-секундного затемнення, `[VO] bright spike: bright=37 avg=3` друкувався щокадру безкінечно навіть через 10+ секунд стабільного яскравого кадру. Небезпечно для польоту: тимчасове перекриття камери (тінь дрона, хмара, пташка) могло б назавжди вимкнути VO. Fix: лічильник `spike_consecutive_frames_` — якщо "spike"-класифікація триває довше за `SPIKE_SUPPRESS` (45 кадрів, ~3с), це вже не спалах, а справжня стала зміна → примусовий re-baseline (`bright_rolling_avg_ = frame_brightness_`), лічильник скидається. Верифіковано на залізі: після рестарту (чистий стан) два цикли прикрити/прибрати об'єктив по 10с — `valid` коректно повертався `true` без deadlock. `camera.h:spike_consecutive_frames_`, `camera_pipeline.cpp` (spike detection block).
69. **Камера змонтована з 90°-поворотом+дзеркалом у новому корпусі → forward-push давав VO-сигнал на dy** — Код мав жорсткий legacy-мапінг: горизонталь кадру (u) = body x/ніс, вертикаль (v) = body y. Новий корпус ставить камеру так, що фізичне "вперед" = низ кадру (+v), "вправо" = право кадру (+u) — це поворот З ВІДБИТТЯМ (det=−1), простими HFLIP/VFLIP (які не роблять 90°) не виправляється. Через це рух уперед реєструвався переважно як dy. Fix: env `JTZERO_AXIS_MAP="m00,m01,m10,m11"` (default identity) — 2×2 матриця image-flow→body-flow; застосовується до filtered пікселного flow ПЕРЕД конвертацією в метри; IMU LK hint (Fix #41) конвертується транспонованою (оберненою для ортонормованої) матрицею body→image. Знаки відкалібровано незалежним SAD-заміром послідовних кадрів через /api/camera/frame (вперед → img dy=−130px; вправо → img dx=−98px): для поточного монтажу `JTZERO_AXIS_MAP=0,-1,-1,0` (у override.conf на Pi). ⚠️ Супутнє правило: стендові VO-тести ТІЛЬКИ з увімкненим FC — без FC сим-IMU генерує синусоїдальний gyro, який через LK hint дає фейковий median flow ±0.45px з періодом ~9с (доведено: з FC статичний фон 0.003px). `camera.h:axis_m00_..m11_`, `camera_pipeline.cpp` (конструктор — парсинг env; process() — матриця + транспонований hint), `setup.sh` (docs).
78. **Hover-дампер душив реальний рух постійно — EMA критерій виходу з hover не бачив bursty per-feature сигнал** — `update_hover_state()` вирішував is_hovering за EMA (α=0.1, ~10-кадровий часовий сталий) від скалярної медіанної магнітуди (`micro_motion_avg`). На щільній-але-нерівномірній текстурі стенду RAWFLOW (Fix #74) підтвердив: окремі тіки чергуються між near0%=0-5% (майже ВСІ фічі бачать рух) і near0%=90-95% (майже всі "застрягли") — EMA згладжує прямо крізь цей bursty патерн і НІКОЛИ не перетинає поріг під час стійкого реального руху, тому `is_hovering=true` тримався у ВСІХ 606 записах тесту незалежно від сили сигналу → дампер ×0.1 (`result.dx *= 0.1f`) і Kalman hover-decay (`kf_vx_ *= 0.85f`/кадр, Fix #53a) душили pose майже повністю. Fix: замінено критерій на EMA (α=0.3, легше згладжування — популяційний голос по ~50-100 фічах вже сам себе усереднює за один тік) частки near-zero фіч за тік (`near_zero_count/valid_flow`, той самий поріг 0.15px що і RAWFLOW), обчислюваної безумовно (не лише під debug-прапорцем) у тому ж циклі що будує `dx_arr/dy_arr`. Поріг `HOVER_STABLE_FRAC_THRESH=0.5`; популяція <5 фіч (`HOVER_MIN_VALID_FLOW`) — тік пропускається (статистично ненадійно). Верифіковано на залізі (4 незалежні прогони, vo_reset+рух+вимір): `/api/vo/trail` нарешті показує спрямоване зміщення (~0.06-0.21м) замість "приклеєного до нуля" (~0.001м) при тому самому внутрішньому `vo_total_distance` 0.9-1.6м. `camera.h:HoverState` (`stable_feature_frac`, `NEAR_ZERO_PX_SQ`, `STABLE_FRAC_ALPHA`, `HOVER_STABLE_FRAC_THRESH`, `HOVER_MIN_VALID_FLOW`), `camera_pipeline.cpp:update_hover_state()` (сигнатура +2 параметри), виклик у `process()`.
80. **Kalman symmetric gain з'їдав bursty VO-сплески між кадрами — asymmetric attack/release gain (v2, v1 self-referential drift відкинуто)** — Fix #79 показав `K≈0.06` (steady-state) тягне `kf_vx_` до КОЖНОГО виміру однаково, включно з хибними нулями від aperture problem. **v1 (відкинуто):** порівнював `|measured|` з ПОТОЧНИМ `|kf_vx_|` для attack/release — self-referential: щойно `kf_vx_` відхилявся від нуля (шум/IMU-bias), дрібний шум завжди виглядав "меншим за kf_vx_" → перманентний release → дрейф компаундиться (кожен наступний шум "менший" за вже-роздутий kf_vx_). Підтверджено на залізі: `kf_vx_` виповз до -0.09 м/с, `kf_vy_` до +0.12 м/с за ~15с при `raw_vx/vy≈0.0001-0.005` (без реального руху) — `/api/vo/trail` показав 0.71м чистого дрейфу. **v2 (задеплоєно):** attack/release вирішується АБСОЛЮТНИМ порогом (`SPIKE_THRESH_MPS=0.01` м/с — чи сам вимір достатньо великий, не порівняння з `kf_vx_`) — розриває петлю зворотного зв'язку. `MAX_RELEASE_FRAMES=30` (~2с) — safety cap, примусова повна корекція після стількох поспіль release-кадрів, обмежує найгірший дрейф навіть при сталому підпороговому зсуві. `RELEASE_FACTOR=0.15`. Variance-апдейт (`kf_vx_var_ *= (1-Kx)`) навмисно лишився на НЕ-масштабованому Kx. Верифіковано на залізі: тест спокою 12с → дрейф 0.0001м (був би 0.05-0.1м+ з v1); 4 живі тести руху → pass-through ratio 44%/15.5%/24.8%/41.1% (було 0.4-38% з Fix #78/#78b), без дрейфу в жодному. Варіативність пояснена освітленням (2 ліхтарі, слабше світло → нижчий ratio) — узгоджується з механізмом (менше aperture problem при кращому контрасті). `camera.h:release_count_x_/y_` (нові приватні лічильники, скидаються в `reset()`), `camera_pipeline.cpp` (Kalman update блок, `[KFDBG]` розширено полем `Keff`).

79. **(діагностика) Справжній корінь ~85% втрати сигналу знайдено: Kalman R/Q баланс, не hover-гейт** — Додано `[KFDBG]` лог (env-gated `JTZERO_VO_DEBUG=1`, throttle кожен 3-й тік) одразу після Kalman update — друкує `raw_vx/vy` (VO вимір), `kf_vx_/vy_` (пост-апдейт стан), `Kx/Ky` (gain), `R`, `Q`, `hov`. Живий тест на залізі показав: `Kx/Ky≈0.06` у спокої (`R=0.300`, `Q=0.0011` — сталі LOW zone) — фільтр з часовою константою ~1-1.5с. Реальний VO-сигнал на bursty-текстурі приходить сплесками по 1-2 кадри (66-130мс, aperture problem). Один сплеск підштовхує `kf_vx_` лише частково (на `K`), наступні "тихі" кадри так само частково тягнуть його НАЗАД до нуля (`kf_vx_ += K×(0-kf_vx_)`) — `kf_vx_` не встигає утриматись на плато реальної швидкості між сплесками, тому `Σ(kf_vx_×dt)` (накопичена поза) виходить набагато меншим за реальне переміщення. Підтверджено на живих даних: `kf_vx_` стрибав до -0.11 м/с на сплеску, потім плавно згасав до нуля наступні кадри при `raw_vx≈0`. **Це і є справжня причина ~15-40% pass-through у A3-тестах** (Fix #78/#78b прибрали лише hover-гейт, який був другорядним чинником). Наступний крок: переглянути R/Q баланс (можливо адаптивний R, що падає різкіше при сплеску, чи інша структура фільтра, толерантна до bursty-сигналу) — НЕ зроблено, потребує окремого проєктного рішення, не швидкого фіксу. `camera_pipeline.cpp` (KFDBG print, одразу після Kalman update, ~рядок 880).

78b. **HOVER_MIN_FRAMES=30 (~2с) недостатньо проти bursty гепів під час руху — підняв до 90 (~6с), ефект менший за очікуваний** — Аналіз RAWFLOW/VODBG логу Fix #78 показав мід-push "виглядає стабільним" геп ~3с (20:20:44-47 у тестовому вікні), достатній щоб знову підтвердити `is_hovering=true` (поріг 30 кадрів) і повторно запустити Kalman decay (+1с) та pose-дампер (+2с) посеред безперервного фізичного руху. Підняв поріг з запасом над виміряним гепом. **Результат неоднозначний:** серія A3-тестів (7 прогонів, vo_reset+точно виміряний 30см push+вимір `/api/vo/trail`) дала ratio (trail-delta / internal vo_total_distance) 0.4%→14%→17%→22%→22%→38%→38% — суттєвого й стабільного покращення НЕ підтверджено, підняття порогу саме по собі не було домінантним важелем. **Ключова знахідка натомість:** найкращі результати (38%, майже чиста одна вісь) дав НАЙШВИДШИЙ push (10с замість 15с) — короткий безперервний рух рідше встигає "виглядати стабільним" достатньо довго для повторного спрацювання дампера. Ratio й далі суттєво нижчий за очікувані ~100% (для ±20% допуску по магнітуду) — залишок втрати НЕ пояснюється лише hover-гейтом; підозра на сам Kalman R/Q баланс (`kf_vx_`/`kf_vy_`), потребує окремого логування поряд з `raw_vx/raw_vy` per-tick (не зроблено). `camera.h:HoverState::HOVER_MIN_FRAMES`.
