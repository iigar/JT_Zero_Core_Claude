...
54. **rpicam-vid auto-exposure: conf=0.00 при bright=181 (засвітка)** — Pi Camera v2 auto-exposure адаптується до темряви, потім при яскравому освітленні (сонце, лампи) дає засвічені кадри (bright>150) → FAST/LK не знаходять градієнтів → conf=0.00. Також: frame-to-frame зміна яскравості при auto-exposure порушує LK tracker (змінюється інтенсивність пікселів між кадрами). Причина низького conf на землі до польоту: camera pointing down, ~5см від підлоги, ground_dist=1.0м → raw_vx переоцінений в 10-20× → imu_consistency=0.1 (підлога) → conf≈3%. Fix: `--shutter 8000 --gain 4.0` в rpicam-vid команді (8мс фіксована витримка, gain 4.0). Фіксована витримка гарантує однакову яскравість кадрів → LK стабільний. `jt-zero/camera/camera_drivers.cpp:126`.
