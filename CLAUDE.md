...
55. **vo_valid per-frame flip → EKF3 дрейф під час польоту** — `result.valid = running_confidence_ > 0.32 && inlier_count >= min` перевірявся per-frame без гістерезису. Під час нормального польоту conf коливається (0.13-0.59): кожен провал нижче 0.32 миттєво зупиняв VISION_POSITION_ESTIMATE → EKF3 переходив на IMU-only → дрон дрейфував у LOITER. Підтверджено на реальному польоті в кімнаті. Fix: debounce через `vo_valid_stable_` (new member): `false` тільки після 5 підряд поганих кадрів (333ms), `true` після 2 підряд хороших (133ms). Асиметрія: швидке відновлення, повільний dropout. `camera.h` (3 нових члени + 2 constexpr), `camera_pipeline.cpp:~842`.
