...
45. **accumulate_gyro() не викликається при fc_active=true — LK hints мертві з реальним FC** — `sensor_loop` (T1) викликає `accumulate_gyro()` лише у гілці `if (!fc_active)`. Коли FC підключений (`mavlink_.has_fc_data()=true`), весь блок пропускається — `preint_.valid` залишається false назавжди → LK hints ніколи не застосовуються → TRK падає до 0-4 при будь-якому yaw маневрі незалежно від якості gyro даних з FC. Виявлено апаратним тестом: Pi + Matek H743, `/api/sensors` → `imu: mavlink`, маневр yaw → TRK: 0-4 замість очікуваних 80+. Fix: у гілці `else` (fc_active=true) читаємо gyro з `state_.imu` (заповнює T5 з SCALED_IMU) під `sensor_mutex_` і передаємо в `accumulate_gyro()`. T1 читає те саме значення 4-8× між MAVLink оновленнями (FC @ 25-50Hz, T1 @ 200Hz), але `sum(rate × dt) = rate × total_time` → правильна накопичена кутова похибка. `runtime.cpp:sensor_loop`.
