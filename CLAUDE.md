...
55. **vo_valid поріг 0.32 зупиняє VO під час нормального польоту → EKF3 drift** — Під час активного польоту conf стабільно 0.15-0.35 (imu_consistency коливається при реальних прискореннях). Поріг 0.32 вимикав VISION_POSITION_ESTIMATE на більшість часу польоту → EKF3 тайм-аутив → дрон дрейфував у LOITER. Geometrically: 30+ inliers при conf=0.20 — валідна позиція, просто з більшою невизначеністю. EKF3 обробляє це через covariance weighting, а не вмикання/вимикання джерела. Fix A: debounce `vo_valid_stable_` — false після 5 підряд поганих кадрів (333ms), true після 2 хороших. Fix B: поріг 0.32 → 0.15 — дозволяє VO при conf=0.16+ в польоті, EKF3 вагує за covariance. `camera_pipeline.cpp:784`, `camera.h` (debounce members).
